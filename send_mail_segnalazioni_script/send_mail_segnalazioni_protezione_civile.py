...
logger = logging.getLogger('settore_protezione_civile_segnalazione')
hdlr = logging.FileHandler('/var/tmp/settore_protezione_civile_segnalazione.log')
...
def doQuery( conn ) :
    ...
    TO = 'protezionecivile.toscana@regione.toscana.it'
    FROM ='dbsegnalazioni@lamma.rete.toscana.it'
    BCC = 'mari@lamma.rete.toscana.it'
    
    cur = conn.cursor()

    cur.execute( "SELECT ogg_segn, data_prot_arr, prot_arrivo, codsegn, data_inizio_istr, data_fine_istr, email_sent FROM settore_protezione_civile_segnalazione" )
    ...
    for ogg_segn, data_prot_arr, prot_arrivo, codsegn, data_inizio_istr, data_fine_istr, email_sent in cur.fetchall() :
        ...
        if (giorniScadenza.total_seconds() <= 0.0 and data_fine_istr is None and email_sent == False):
            ...
            cur.execute("UPDATE settore_protezione_civile_segnalazione set email_sent = TRUE where codsegn='"+codsegn+"'");
