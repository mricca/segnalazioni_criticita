...
logger = logging.getLogger('difesa_del_suolo_criticita_segnalazione')
hdlr = logging.FileHandler('/var/tmp/difesa_del_suolo_criticita_segnalazione.log')
...
def get_user_suffix(x):
        return {
            'valdarno_superiore': 'geniocivile.valdarnosuperiore@regione.toscana.it',
            'toscana_nord': 'geniocivile.toscananord@regione.toscana.it',            
            'valdarno_inferiore': 'geniocivile.costa@regione.toscana.it',
            'toscana_sud': 'geniocivile.toscanasud@regione.toscana.it',
...
def doQuery( conn ) :
    ...
    cur.execute( "SELECT ogg_segn, data_prot_arr, prot_arrivo, codsegn, data_inizio_istr, data_fine_istr, email_sent, auth_user.username AS utente FROM difesa_del_suolo_criticita_segnalazione INNER JOIN auth_user ON difesa_del_suolo_criticita_segnalazione.utente_id = auth_user.id;" )

    for ogg_segn, data_prot_arr, prot_arrivo, codsegn, data_inizio_istr, data_fine_istr, email_sent, utente in cur.fetchall() :
        ...
        if (giorniScadenza.total_seconds() <= 0.0 and data_fine_istr is None and email_sent == False):
            ...
            #print TO
            cur.execute("UPDATE difesa_del_suolo_criticita_segnalazione set email_sent = TRUE where codsegn='"+codsegn+"'");
