...
logger = logging.getLogger('gc_toscana_nord_segnalazione')
hdlr = logging.FileHandler('/var/tmp/gc_toscana_nord_segnalazione.log')
...
def doQuery( conn ) :
    ...
    cur.execute( "SELECT ogg_segn, data_prot_arr, prot_arrivo, codsegn, data_inizio_istr, data_fine_istr, email_sent FROM gc_toscana_nord_segnalazione" )
    ...
    for ogg_segn, data_prot_arr, prot_arrivo, codsegn, data_inizio_istr, data_fine_istr, email_sent in cur.fetchall() :
        ...
        if (giorniScadenza.total_seconds() <= 0.0 and data_fine_istr is None and email_sent == False):
            ...
            cur.execute("UPDATE gc_toscana_nord_segnalazione set email_sent = TRUE where codsegn='"+codsegn+"'");
