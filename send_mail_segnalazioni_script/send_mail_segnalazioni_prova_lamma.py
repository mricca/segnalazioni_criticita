...
logger = logging.getLogger('prova_lamma')
hdlr = logging.FileHandler('/var/tmp/prova_lamma.log')
...
def doQuery( conn ) :
    ...
    TO = 'rmari76@gmail.com'
    FROM ='dbsegnalazioni@lamma.rete.toscana.it'
    BCC = 'mari@lamma.rete.toscana.it'

    cur = conn.cursor()

    cur.execute( "SELECT ogg_segn, data_prot_arr, prot_arrivo, codsegn, data_inizio_istr, data_fine_istr, email_sent FROM prova_lamma_segnalazione" )
