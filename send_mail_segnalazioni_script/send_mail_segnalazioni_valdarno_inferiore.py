...
logger = logging.getLogger('gc_valdarno_inferiore_e_costa_segnalazione')
hdlr = logging.FileHandler('/var/tmp/gc_valdarno_inferiore_e_costa_segnalazione.log')
...
def doQuery( conn ) :
    ...
    TO = 'geniocivile.costa@regione.toscana.it'
    FROM ='dbsegnalazioni@lamma.rete.toscana.it'
    BCC = 'mari@lamma.rete.toscana.it'
    
    cur = conn.cursor()

    cur.execute( "SELECT ogg_segn, data_prot_arr, prot_arrivo, codsegn, data_inizio_istr, data_fine_istr, email_sent FROM gc_valdarno_inferiore_e_costa_segnalazione" )
    ...
    for ogg_segn, data_prot_arr, prot_arrivo, codsegn, data_inizio_istr, data_fine_istr, email_sent in cur.fetchall() :
        ...
        if (giorniScadenza.total_seconds() <= 0.0 and data_fine_istr is None and email_sent == False):
            ...
            cur.execute("UPDATE gc_valdarno_inferiore_e_costa_segnalazione set email_sent = TRUE where codsegn='"+codsegn+"'");
