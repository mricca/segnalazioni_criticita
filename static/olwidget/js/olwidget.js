...
var olwidget = {
     ...
     */
     updateFieldsCoordsApp: function(geom){
        if(document.getElementById("id_coord_x") && document.getElementById("id_coord_y") && document.getElementById("id_lat") && document.getElementById("id_lon")){
            if(!geom){
                document.getElementById("id_coord_x").value = 0;
                document.getElementById("id_coord_y").value = 0;
                document.getElementById("id_lat").value = 0;
                document.getElementById("id_lon").value = 0;
            }else{            
                var projWGS84 = new OpenLayers.Projection("EPSG:4326");
                var proj3003 = new OpenLayers.Projection("EPSG:3003");

                var point = new OpenLayers.LonLat(geom.geometry.x,geom.geometry.y);

                var coord_x = parseFloat(geom.geometry.x);
                var coord_y = parseFloat(geom.geometry.y);
                
                var point2 = point.transform(proj3003,projWGS84);

                document.getElementById("id_coord_x").value = Math.round10(coord_x,-4);
                document.getElementById("id_coord_y").value = Math.round10(coord_y,-4);
                document.getElementById("id_lat").value = Math.round10(point2.lat,-6);
                document.getElementById("id_lon").value = Math.round10(point2.lon,-6);
            }
        }
     },
    /*
    ...
    featureToEWKT: function(feature, proj) {
        this.updateFieldsCoordsApp(feature);
    ...
    ewktToFeature: function(wkt) {
        ...
        var geom = this.wktFormat.read(wkt);
        //this.updateFieldsCoordsApp(geom);
        return geom;
    },
...
olwidget.Map = OpenLayers.Class(OpenLayers.Map, {
    ...
    initMap: function(mapDivId, opts) {
        ...
        for (var i = 0; i < layers.length; i++) {
            //if (layers[i].name != "Comprensori RT" && layers[i].name != "Reticolo di gestione DCR 57/2013" && layers[i].name != "Comuni" && layers[i].name != "Province" && layers[i].name != "Italia" && layers[i].name != "Regione Toscana" && layers[i].name != "Anno 2013 col - AGEA" && layers[i].name != "CTR 1:10000") {
            if (layers[i].name != "Comprensori RT" &&
                //layers[i].name != "Comuni" &&
                layers[i].name != "Province" &&
                layers[i].name != "Italia" &&
                layers[i].name != "Regione Toscana" &&
                layers[i].name != "Anno 2013 col - AGEA" &&
                layers[i].name != "CTR 1:10000") {
                var control;
...
olwidget.EditableLayer = OpenLayers.Class(olwidget.BaseVectorLayer, {
    ...
    clearFeatures: function() {
        olwidget.updateFieldsCoordsApp();
    ...
    readWKT: function() {
        ...
        if (wkt) {
            ...
            if (!olwidget.isCollectionEmpty(geom)) {
                ...
                olwidget.updateFieldsCoordsApp(geom);
