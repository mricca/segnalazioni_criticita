...
class Segnalazione(gismodels.Model):
    ...
    # OBBLIGATORIO DI DEFAULT
    tipologia_richiesta = gismodels.ForeignKey(TipologiaRichiesta, blank=True, null=True, related_name='tipologia_richiesta_settore_protezione_civile',db_index=True, db_column='tipo_rich')
    ...
    # OBBLIGATORIO DI DEFAULT
    oggetto_segnalazione = gismodels.ForeignKey(OggettoSegnalazione, blank=False, null=True, related_name='oggetto_segnalazione_settore_protezione_civile',db_index=True, db_column='ogg_segn')
    ...
    # OBBLIGATORIO DI DEFAULT
    bacino_idrografico = gismodels.ForeignKey(Bacini, blank=True, null=True, related_name='bacini_settore_protezione_civile', db_column='bacidro',db_index=True, help_text='Bacino idrografico L 183/1989')
    
    # OBBLIGATORIO DI DEFAULT
    provincia = gismodels.ForeignKey(Province, blank=True, null=True,db_index=True, related_name='provincia_settore_protezione_civile')
    
    # OBBLIGATORIO DI DEFAULT
    #comune = gismodels.ForeignKey(Comuni, blank=False, null=True, related_name='comune_lamma')
    comune = ChainedForeignKey(Comuni, chained_field="provincia", chained_model_field="cod_prov",show_all=False, auto_choose=True, blank=True, null=True,db_index=True, related_name='comune_settore_protezione_civile')
    ...
    # OBBLIGATORIO DI DEFAULT
    motivo_segnalazione = gismodels.ForeignKey(MotivoSegnalazione, blank=False, null=True, related_name='motivo_segnalazione_settore_protezione_civile', db_column='mot_segn')
    
    # OBBLIGATORIO DI DEFAULT
    inserimento_dods = gismodels.ForeignKey(InserimentoDODS, blank=True, null=True, related_name='inserimento_dods_settore_protezione_civile', db_column='ins_dods', verbose_name="Inserimento nel Documento Operativo")
    ...
    # OBBLIGATORIO DI DEFAULT
    set_reg_comp = gismodels.ForeignKey(SettoreRegComp, blank=False, null=True, related_name='set_reg_comp_settore_protezione_civile', db_column='set_reg_comp', verbose_name="Settore regionale competente")
...
class DocumentazioneCollegataPc(models.Model):
    ...
    documento_collegato = gismodels.FileField(upload_to='documents/documenti_collegati_pc/%Y/%m/%d',blank=True, null=True, db_column='doc_coll')
    relate = gismodels.ForeignKey(Segnalazione, db_column='segn_fk', blank=False, null=True)
    
    class Meta:
        db_table = 'documentazione_pc'   
