...
from models import Segnalazione, DocumentazioneCollegataVi
...
class DocumentazioneCollegataViAdminForm(forms.ModelForm):

    def __init__(self, *args, **kwargs):
            self.request = kwargs.pop('request', None)
            super(DocumentazioneCollegataViAdminForm, self).__init__(*args, **kwargs)
            
    class Meta:
        model = DocumentazioneCollegataVi
        fields = '__all__'
        widgets = {
...
def export_pdf_single(modeladmin, request, queryset, cod = None):
    ...
    response['Content-Disposition'] = 'attachment; filename="Segnalazione_criticita.pdf"'
    ...
    header = Paragraph('''<para align=center spaceb=3><b><font color=red >Segnalazione Criticità</font></b></para>''', style)
...
def export_csv(modeladmin, request, queryset):
    ...
    response['Content-Disposition'] = 'attachment; filename=segnalazione_criticita.csv'
...
def export_xls(modeladmin, request, queryset):
    ...
    response['Content-Disposition'] = 'attachment; filename=segnalazione_criticita.xls'
    wb = xlwt.Workbook(encoding='utf-8')
    ws = wb.add_sheet("Segnalazione criticita")
...
class DocumentazioneCollegataViAdmin(GeoModelAdmin):
    
    form = DocumentazioneCollegataViAdminForm
...
class DocumentazioneCollegataViInline(admin.TabularInline):
    model = DocumentazioneCollegataVi
...
class SegnalazioneAdmin(GeoModelAdmin):
    ...
    fieldsets = [
        ...
        ("DOCUMENTAZIONE COLLEGATA", {"classes": ("placeholder documentazionecollegatavi_set-group",), "fields" : ()}),(
        'STATO ISTRUTTORIA', 
            {'classes': ('grp-collapse grp-open',),'fields': 
                [
                'stato_istruttoria',
    ...
    inlines = [DocumentazioneCollegataViInline]
    ...
    def save_model(self, request, obj, form, change):
        ...
        if not queryset:
            #codice_segnalazione = 'DA' + '2014' + 'LA' + '0001'
            codice_segnalazione = 'SEGN-' + 'VI' + '2016' + '00001'
            ...
            codice_segnalazione = 'SEGN-' + 'VI' + '2016' + ultimo[0]
...
#admin.site.register(DocumentazioneCollegataVi, DocumentazioneCollegataViAdmin)